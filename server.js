...
// Phục vụ index.html và style.css
app.use(express.static(path.join(__dirname)));

// Hàm trích xuất token linh hoạt hơn (Header hoặc Cookie)
function getToken(req) {
  ...
  // Fallback: Lấy token từ Cookie nếu có
...
app.post("/api/auth/logout-all", requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Không thể đăng xuất tất cả thiết bị."
    });
  }
});
...
// Fallback phục vụ Frontend SPA (Ví dụ: React Router/HTML Direct Routing)
app.get("*", (req, res) => {
...
async function startServer() {
  try {
    await initDatabase();
    app.listen(PORT, () => {
      console.log(`KAISOUL ID running on port ${PORT}`);
    });
  } catch (error) {
